...
def download_data(csv_url: str,year,month,color):
    # the backup files are gzipped, and it's important to keep the correct extension
    # for pandas to be able to open the file
    if csv_url.endswith('.csv.gz'):
        csv_name = f"{color}_{year}_{month}.csv.gz"
    else:
        csv_name = f"{color}_{year}_{month}.csv"
    os.system(f"wget {csv_url} -O {csv_name} -P C:\dataengeneering\zoomcamp\week_4")
    return(csv_name)
...
@flow
def main_flow(year,months,color):

    for month in months:
        dataset_file = f"{color}_tripdata_{year}-{month:02}"
        table_name = f"{color}_{year}-{month:02}"
        
        csv_url = f"https://github.com/DataTalksClub/nyc-tlc-data/releases/download/{color}/{dataset_file}.csv.gz"
        

        log_subflow(table_name)
        csv_name = download_data(csv_url,year,month,color)
