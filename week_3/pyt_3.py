...
def download_data(csv_url: str,year,month):
    # the backup files are gzipped, and it's important to keep the correct extension
    # for pandas to be able to open the file
    if csv_url.endswith('.csv.gz'):
        csv_name = f"fhv_{year}_{month}.csv.gz"
    else:
        csv_name = f"fhv_{year}_{month}.csv"
...
@flow
def main_flow(year,months):

    for month in months:
        dataset_file = f"fhv_tripdata_{year}-{month:02}"
        table_name = f"fhv_{year}-{month:02}"
        csv_url = f"https://github.com/DataTalksClub/nyc-tlc-data/releases/download/fhv/{dataset_file}.csv.gz"
        

        log_subflow(table_name)
        csv_name = download_data(csv_url,year,month)
