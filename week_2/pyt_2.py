...
def download_data(csv_url: str,color,year,month):
    ...
    os.system(f"wget {csv_url} -O {csv_name} -P C:\dataengeneering\zoomcamp\week_2")
    return(csv_name)
...
@flow
def main_flow(color,year,months):

    for month in months:
        dataset_file = f"{color}_tripdata_{year}-{month:02}"
        table_name = f"{color}_{year}-{month:02}_trips"
        csv_url = f"https://github.com/DataTalksClub/nyc-tlc-data/releases/download/{color}/{dataset_file}.csv.gz"

        log_subflow(table_name)
        csv_name = download_data(csv_url,color,year,month)
